...
import { FaChevronLeft, FaChevronRight, FaHome, FaList, FaPlus, FaAngleDown, FaAngleRight } from 'react-icons/fa';
...
function Navbar() {
    ...
    useEffect(() => {
        ...
        navItems.forEach(item => {
            if (isNavItem(item) && item.children) {
                const isActive = location.pathname.startsWith(item.path) || item.children.some(child => location.pathname.startsWith(child.path));
                ...
                return (
                    <li key={item.path} className="flex flex-col group">
                        <Button asChild>
                            <Link 
                                to={item.path} 
                                className={`text-white hover:text-gray-400 flex items-start justify-start ${isExpanded ? '' : 'justify-center'}`}
                                onClick={() => toggleItemExpansion(item.path)}
                            >
                                <NavIcon className={`text-sm ${isExpanded ? 'mr-2' : ''}`} />
                                {isExpanded && <span>{item.label}</span>}
                                {item.children && isExpanded && (
                                    <span className={`ml-auto transition-transform duration-300 ${isItemExpanded ? 'rotate-180' : ''}`}>
                                        <FaAngleDown />
                                    </span>
                                )}
                            </Link>
                        ...
                        {item.children && isExpanded && (
                            <ul className={`ml-4 mt-2 space-y-2 transition-all duration-300 ${isItemExpanded ? 'max-h-screen' : 'max-h-0 overflow-hidden'} group-hover:max-h-screen`}>
    ...
    return (
        <nav className={`flex flex-col bg-black h-screen p-4 transition-width duration-300 ${isExpanded ? 'w-60' : 'w-20'}`}>
