    ...
    return (
        <div className="overflow-x-auto">
            {tools.length > 0 ? (
                <>
                    <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-md text-sm">
                        <thead className="bg-black text-white">
                            <tr>
                                <th className="py-2 px-3 border-b text-left font-semibold">Tool Number</th>
                                <th className="py-2 px-3 border-b text-left font-semibold">Description</th>
                                <th className="py-2 px-3 border-b text-left font-semibold">Quantity</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginatedTools.map((tool) => (
                                <tr key={tool.id} className="hover:bg-gray-100 transition-colors duration-200">
                                    <td className="py-2 px-3 border-b text-black">{tool.toolNumber}</td>
                                    <td className="py-2 px-3 border-b text-black">{tool.toolDescription}</td>
                                    <td className="py-2 px-3 border-b text-black">{tool.quantity}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <p className="text-black">No tools available for this project.</p>
            )}
        </div>
    );
};
